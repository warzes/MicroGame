...
namespace temp
{
	...
	struct Renderer
	{
		...
		// Get the mouse ray.
		glm::vec3 getMouseRay(temp::Camera camera, int mouseX, int mouseY)
		{
			float u = float(mouseX) * 2.0f / float(GetRenderWidth()) - 1.0f;
			float v = 1.0f - float(mouseY) * 2.0f / float(GetRenderHeight());
			glm::vec4 b = glm::inverse(projection) * glm::vec4(u, v, -1.0f, 1.0f);
			return glm::normalize(glm::inverse(camera.getView()) * glm::vec4(b.x, b.y, -1.0f, 0.0f));
		}
