...
namespace g3d
{
	...
	void Model::SetInstancedBuffer(VertexBuffer* instanceBuffer, const std::vector<VertexAttribute>& attribs)
	{
		for (int i = 0; i < m_subMeshes.size(); i++)
		{
	...
	void drawPrimitive::DrawLine(const Camera& camera, const glm::vec3& startPos, const glm::vec3& endPos)
	{
		...
		if (!isCreate)
		{
			...
			{
				{.size = 3, .type = VertexAttributeType::Float, .normalized = false, .stride = 0, .pointer = (void*)0},
			};
